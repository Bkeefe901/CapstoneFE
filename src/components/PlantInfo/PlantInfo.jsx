...
export default function PlantInfo({ plants, setNewList }) {
    ...
    async function handleClick(ob) {
        ...
        try {
            await axios.post(`https://garden-tracker-0rj8.onrender.com/userplant`, plantToAdd, options);
    ...
    async function handleDelete(ob) {
        ...
        if (userConfirmed) {
            try {
                console.log(options);
                await axios.delete(`https://garden-tracker-0rj8.onrender.com/plant/${ob._id}`, config);
